...
void _free_tree(BSTNode* root) {
    ...
    free_element(root->element); // Free the element
    free(root->key);
    free(root);
}
...
bool parse_object(BSTNode** a_root, const char** a_pos) {
    if (a_root == NULL || a_pos == NULL || *a_pos == NULL) {
        return false;
    }
    ...
    *a_root = NULL;
    ...
    while (true) {
        ...
        if (!parse_string(&key, a_pos)) {
	//		printf("\n%s\n", key);
            _free_tree(*a_root); // Free the binary search tree if an error occurs
	//		printf("key err");
            return false;
        }

        // Eat whitespace after the key
        while (isspace(**a_pos)) {
            (*a_pos)++;
        }
	...
	//	printf("post tree");
        ...
        if (*a_root == NULL) {
            *a_root = new_node;
            ...
            BSTNode* current = *a_root;
            while (true) {
                int cmp_result = strcmp(key, current->key);
                if (cmp_result == 0) {
                    // Key already exists, replace the value in the old node with the new element
                    free_element(current->element);
                    current->element = element;
                    free(key); // Free the key since it's already in the tree
                    free(new_node); // Free the new node since it's not used
                    break;
	...
	//	printf("end");
        // Check if it's the end of the object or there's a comma separator
        if (**a_pos == '}') {
            (*a_pos)++; // Move past the '}'
	//		printf("true");
            return true;
...
Element* get_element(BSTNode* root, const char* key) {
    ...
    if (cmp_result == 0) {
        // Key found, return the address of the element in the node
        return &(root->element);
    } else if (cmp_result < 0) {
...
void print_element_to_file(Element element, FILE* file) {
	...
	switch (element.type) {
		...
		case ELEMENT_OBJECT:
			fprintf(file, "{");
            const BSTNode* node = element.as_object;
            bool first = true;
            while (node != NULL) {
                if (!first) {
                    fprintf(file, ", ");
                ...
                print_element_to_file((node->element), file);
                node = node->right;
                first = false;
            }
            fprintf(file, "}");
...
void free_element(Element element) {
  ...
  //          }
            break;
		case ELEMENT_NULL:
            // Nothing to free for null
            break;
        case ELEMENT_BOOL:
            // Nothing to free for a boolean
            break;
        case ELEMENT_OBJECT:
            // Free the binary search tree
            if (element.as_object != NULL) {
                _free_tree(element.as_object);
            }
            break;
    }
}
